...
public class WebDriverUtils {
	...
	public WebDriverUtils(){
		...
		driver = this.getWebDriver_new(Constant.DEFAULT_BROWSER, false);
	}
	...
	public void implicitWait(final By by, final int timeOut_MilliSecs, final int pollDuration_MilliSecs){
		
		try{
			Wait<WebDriver> varwait = new FluentWait<WebDriver>(driver).withTimeout(timeOut_MilliSecs, TimeUnit.MILLISECONDS)
					.pollingEvery(pollDuration_MilliSecs, TimeUnit.MILLISECONDS).ignoring(NoSuchElementException.class);
	...
	public void implicitWait(final By by) {
		...
		this.implicitWait(by, Constant.IMPLICITWAIT_MILLIS, Constant.IMPLICITWAIT_POLL_MILLIS);
	}
	...
	public WebDriver getWebDriver_new(final Type browser_type,
		...
		try{
			...
			switch (browser_type) {
			case FireFox:
				System.setProperty("webdriver.firefox.bin", Constant.USRDIR + "/driver/Firefox");
	...
	public void fillin_textarea(final WebElement elem, final String str) {
	...
	public void select_selectorByText(final By by, final String str) {
		...
		implicitWait(by);
		highlightElement(by);
		this.select_selectorByText(driver.findElement(by), str);
	}

	public void select_selectorByText(final WebElement elem, final String str) {
		
		new Select(elem).selectByVisibleText(str);
	}
	...
	public String getSelectorValue(final By by, final int index){
		...
		if(this.isElementPresent(by)){
			...
			List<WebElement> options = (List<WebElement>)(List<?>)selector.getOptions().stream().filter(option -> option.isEnabled()).collect(Collectors.toList());
...
//				selector.selectByIndex(index);	
	...
	public void clickEnter(final WebElement elem) {
		
//		elem.sendKeys(Keys.RETURN);
//		elem.sendKeys(Keys.ENTER);
//		elem.sendKeys(arg0);
		Actions action = new Actions(driver); 
		action.sendKeys(elem, Keys.ENTER).build().perform();
	}
	
	public void clickEnter(final By by) {
		
		implicitWait(by);
		this.clickEnter(driver.findElement(by));
	}
	...
	public String getValue(final WebElement elem) {
		
		return this.getAttributeValue(elem, "value");
	}
	...
	public void moveToElementAndHighlight(final WebElement elem) {
